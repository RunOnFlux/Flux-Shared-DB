...
const commandNames = Object.fromEntries(
  Object.entries(consts).filter(([name]) => name.startsWith('COM_')).map(([name, value]) => [value, name])
);
...
class Server {
 constructor(opts) {
  try{
    ...
    this.sequence = 0;
    this.packetCount = 0;
    this.packetInFlight = false;
    this.lastCommand = 'HANDSHAKE';
    this.remotePeer = `${this.socket.remoteAddress || 'unknown'}:${this.socket.remotePort || 'unknown'}`;
    ...
    this.socket.on("error", (err) => {
      this.logFailure('socket error', err);
      ConnectionPool.releaseConnection(this.connId);
    });
    this.socket.on("end", () => {
      log.info(`${this.logContext()} client ended connection`);
      ConnectionPool.releaseConnection(this.connId);
    });
    this.socket.on("close", (hadError) => {
      const bufferedBytes = this.incoming.reduce((total, chunk) => total + chunk.length, 0);
      const level = hadError || bufferedBytes > 0 ? 'warn' : 'info';
      log[level](`${this.logContext()} socket closed hadError=${hadError} bufferedBytes=${bufferedBytes}`);
    });
    this.socket.on('timeout', () => {
      log.warn(`${this.logContext()} socket timeout`);
    });
    // if (this.handleDisconnect) this.socket.on('end', this.handleDisconnect);
    
    this.sendServerHello();
  }catch(err){
    this.logFailure('initialization failed', err);
  }
 }
 
end = () => {
  try{
    log.info(`${this.logContext()} server ending connection`);
    this.socket.end();
    // ConnectionPool.releaseConnection(this.connId);
    // this.operator.sessionQueries[this.connId] = undefined
  }catch(err){
    this.logFailure('ending connection failed', err);
  }
};

 logContext() {
  return `[mysqlServer conn=${this.connId} peer=${this.remotePeer} command=${this.lastCommand} packet=${this.packetCount} seq=${this.sequence} inFlight=${this.packetInFlight}${this.batchStatement ? ` statement=${this.batchStatement}` : ''}]`;
 }

 logFailure(event, err) {
  const details = err && (err.stack || err.message) || String(err);
  log.error(`${this.logContext()} ${event}: code=${err && err.code || 'unknown'} errno=${err && err.errno || 'unknown'} sqlState=${err && err.sqlState || 'unknown'} ${details}`);
 }
 ...
 sendPacket(payload) {
  try {
   ...
   this.logFailure('socket write failed', err);
   throw err;
  }
 }
 ...
 sendDefinitions(definitions, status = {}) {
  ...
  for (let definition of definitions) {
   payload = Buffer.alloc(64 + ['catalog','schema','table','orgTable','name','orgName','default']
     .reduce((size, field) => size + 9 + Buffer.byteLength(String(definition[field] || '')), 0));
   len = 4;
   ...
   // Strings are serialized as UTF-8; keep binary columns as binary.
   len = payload.writeUInt16LE(definition.characterSet === 63 ? 63 : 0x21, len);
 ...
 sendRow(row) {
  let payload = Buffer.alloc(4 + row.reduce((size, cell) => size + (cell == null ? 1
    : 9 + (Buffer.isBuffer(cell) ? cell.length : Buffer.byteLength(String(cell)))), 0));
  let len = 4;
  for (let cell of row) {
   if (cell == null) {
 ...
 handleData = (data) => {
  try{
    ...
    this.gatherIncoming();
    if (data == null) {
    log.info(`${this.logContext()} no data; destroying socket`);
    this.socket.destroy();
    }
  }catch(err){
    this.logFailure('receiving data failed', err);
  }
 }
 ...
 gatherIncoming() {
  try{
    ...
    this.logFailure('packet framing failed', err);
  }
 }
 ...
 readPackets(buf) {
  ...
  while (true) {
   ...
   this._packetQueue = this._packetQueue.then(() => {
     ...
     this.packetCount++;
     this.packetInFlight = true;
     this.lastCommand = this.onPacket === this.helloPacketHandler ? 'HANDSHAKE'
       : `${commandNames[cmdByte] || 'UNKNOWN'}(0x${cmdByte.toString(16).padStart(2,'0')})`;
     if (typeof this.onPacket !== 'function') {
       this.logFailure('packet dropped', new Error(`No packet handler; packetLength=${packetLength}`));
       return;
     }
     return Promise.resolve(this.onPacket(capturedPacket));
   }).catch((err) => {
     this.logFailure(`packet handler failed (packetLength=${packetLength})`, err);
   }).finally(() => {
     this.packetInFlight = false;
   });
  }
 ...
 helloPacketHandler = (packet) => {
  ...
  .catch( (err) => {
   this.logFailure('authorization or connection allocation failed; destroying socket', err);
 ...
 async _sendBinaryResultSet(sql, connId) {
  ...
  try {
   ...
   if (seqAfterAwait !== seqAtEntry) {
    this.logFailure('response sequence changed during query', new Error(`Expected sequence ${seqAtEntry}, got ${seqAfterAwait}`));
   }
   if (!result) {
   ...
   this.sendError({ message: (err && err.message) || 'Query error', errno: (err && err.errno) || 1105, sqlState: (err && err.sqlState) || 'HY000', error: err });
  }
 }
 ...
 sendOK({ message, affectedRows = 0, insertId = 0, warningCount = 0, serverStatus = consts.SERVER_STATUS_AUTOCOMMIT}) {
  let data = Buffer.alloc(Buffer.byteLength(message) + 64);
  let len = 4;
  len = data.writeUInt8(0,len);
  len = writeLengthCodedBinary(data,len,affectedRows);
  len = writeLengthCodedBinary(data,len,insertId);
  len = data.writeUInt16LE(serverStatus,len);
 ...
 sendError({ message = 'Unknown MySQL error',errno = 2000,sqlState = "HY000", error}) {
  //## Sending Error ...
  this.logFailure('sending error response', error || { message, errno, sqlState });
  let data = Buffer.alloc(Buffer.byteLength(message) + 64);
  let len = 4;
...
function writeLengthCodedString(buf,pos,str) {
 ...
 if (len < 251) {
  ...
  if (Buffer.isBuffer(str)) str.copy(buf, pos + 1); else buf.write(str, pos + 1);
  return pos + len + 1;
 } else if (len < 65536) {
  buf.writeUInt8(0xFC, pos);
  buf.writeUInt16LE(len, pos + 1);
  if (Buffer.isBuffer(str)) str.copy(buf, pos + 3); else buf.write(str, pos + 3);
  return pos + len + 3;
 } else {
  buf.writeUInt8(0xFD, pos);
  buf.writeUIntLE(len, pos + 1, 3);
  if (Buffer.isBuffer(str)) str.copy(buf, pos + 4); else buf.write(str, pos + 4);
  return pos + len + 4;
 }
}
...
function writeLengthCodedBinary(buf,pos,number) {
 if (number == null) {
  ...
  buf.writeBigUInt64LE(BigInt(number),pos + 1);
  return pos + 9;
 }
}

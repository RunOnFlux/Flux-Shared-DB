...
function getQueryType(sql) {
  const readFlags = ['select', 'show', 'describe'];
  if (sql.toLowerCase().startsWith(readFlags[0]) || sql.toLowerCase().startsWith(readFlags[1]) || sql.toLowerCase().startsWith(readFlags[3])) {
    // it's a read query
    return 'r';
  }
  // it's a write query

...
// Create HTTPS agent that accepts insecure certificates
...
async function getApplicationSpecs(appName) {
  try {
    const fluxnodeList = await axios.get(`https://api.runonflux.io/apps/appspecifications/${appName}`, {
      timeout: 13456,
      httpsAgent,
    });
...
async function getApplicationIP(appName) {
  try {
    const fluxnodeList = await axios.get(`https://api.runonflux.io/apps/location/${appName}`, {
      timeout: 13456,
      httpsAgent,
    });
    if (fluxnodeList.data.status === 'success') {

/* eslint-disable no-undef */
/* eslint-disable no-unused-vars */
/**
* [This Library helps analyze sql queries and seperate read/write queries for mysql, mssql and postgre]
*/
const { identify } = require('sql-query-identifier');
const log = require('./log');
const config = require('../ClusterOperator/config');

/**
 * [getQueryType analyzes a single query and returns r or w for query type]
 * @param {string} sql sql query
 * @return {string}        [r/w]
 */
function getQueryType(sql) {
  // Connection-local controls must execute on the assigned DB connection but
  // must never be stored as replicated data changes.
  const readFlags = ['select', 'show', 'describe', 'set names', 'set transaction', 'kill', 'set profiling'];
  // eslint-disable-next-line no-restricted-syntax
  for (const flag of readFlags) { if (sql.toLowerCase().startsWith(flag)) return 'r'; }
  // disable transient filtering for test apps (remove the whole block after tests are done)
  if (!config.containerDataPath.includes('s:/app/dumps')) {
    if (sql.includes('`wp_options`') && sql.includes('_transient_')) {
    //  log.warn('wp transient query');
      return 'r';
    }
  }
  if (sql.toLowerCase().startsWith('set session')) {
    return 's';
  }

  // it's a write query
  return 'w';
}
function removeFirstLine(str) {
  const lines = str.split('\n');
  lines.splice(0, 1);
  return lines.join('\n');
}
/**
 * [clean up linebraks]
 * @param {string} sql sql query
 */
function cleanUP(sql) {
  // log.query(sql);
  // fixes mysql2 client utf8 support issue
  // eslint-disable-next-line no-param-reassign
  if (sql.toLowerCase().startsWith('/*!40101 set character_set_client = utf8 */')) sql = '/*!40101 SET character_set_client = utf8mb4 */';
  // eslint-disable-next-line no-param-reassign
  if (sql.toLowerCase().startsWith('set character_set_client = utf8')) sql = 'SET character_set_client = utf8mb4';
  // eslint-disable-next-line no-param-reassign
  if (sql.toLowerCase().startsWith('/*!40101 set names utf8 */')) sql = '/*!40101 SET NAMES utf8mb4 */';
  // eslint-disable-next-line no-param-reassign
  if (sql.toLowerCase().startsWith('set names utf8')) sql = 'SET NAMES utf8mb4';
  // fixes navicat client sending multiple queries after connection
  if (sql.startsWith("SHOW VARIABLES LIKE 'lower_case_%'; SHOW VARIABLES LIKE 'sql_mode'; ")) {
    // eslint-disable-next-line no-param-reassign
    sql = sql.replace("SHOW VARIABLES LIKE 'lower_case_%'; SHOW VARIABLES LIKE 'sql_mode'; ", '');
    // log.debug(sql);
  }
  if (sql.startsWith('--')) {
    // eslint-disable-next-line no-param-reassign
    while (sql.startsWith('--') || sql.startsWith('\r\n') || sql.startsWith('\n')) sql = removeFirstLine(sql);
  }
  return sql;
}
/**
 * [analyzeSql analyzes the sql and marks them as read/write]
 * @param {string} sql sql query
 * @param {string} options sql engine:mysql,mssql,postgre
 * @return {array}        [array of queries]
 */
function analyzeSql(sql, options) {
  // identify() parsing/splitting is only enabled when DB_TYPE=mariadb
  if (config.clientType !== 'mariadb') {
    const raw = cleanUP(sql.trim());
    return raw ? [[raw, getQueryType(raw)]] : [];
  }

  let dialect;
  if (options === 'mysql') {
    dialect = 'mysql';
  } else if (options === 'mssql') {
    dialect = 'mssql';
  } else if (options === 'postgre') {
    dialect = 'psql';
  } else return [];
  const analyzedArray = [];
  let statements;
  try {
    statements = identify(sql.trim(), { dialect, strict: false });
  } catch (e) {
    // fallback: treat whole input as one statement
    statements = [{ text: sql.trim() }];
  }
  // eslint-disable-next-line no-restricted-syntax
  for (const stmt of statements) {
    const tempSql = cleanUP(stmt.text.trim());
    if (tempSql) analyzedArray.push([tempSql, getQueryType(tempSql)]);
  }
  // log.query(sql, 'yellow');
  return analyzedArray;
}

module.exports = analyzeSql;

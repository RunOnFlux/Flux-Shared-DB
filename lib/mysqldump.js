...
// eslint-disable-next-line complexity
function getDataDump(connectionOptions, options, tables, dumpToFile, newConnection) {
  return __awaiter(this, void 0, void 0, function* () {
      ...
      const format$$1 = options.format
          ? (sql) => sqlformatter.format(sql)
          : (sql) => sql;
      // Open a new connection with a special typecast function for dumping data
      ...
      function saveChunk(str, inArray = true) {
          ...
          if (outFileStream) {
              str.forEach(async s => {
                  outFileStream.write(`${s}\n`);
              });
              if (saveChunk.counter === undefined) {
                  saveChunk.counter = 0;
              }
              saveChunk.counter++;
              if (saveChunk.counter % 5000 === 0) {
                  const stats = fs.statSync(outFileStream.path);
                  const fileSizeInBytes = stats.size;
                  const fileSizeInMegabytes = fileSizeInBytes / (1024 * 1024);
                  log.info(`Current dump file size: ${fileSizeInMegabytes.toFixed(2)} MB ...`);
              }
          }
          // Write to memory if configured
          if (inArray && currentTableLines) {
              str.forEach(async s => {
                  currentTableLines.push(s);
              });
          }
      }
      ...
      try {
          ...
          while (tables.length > 0) {
              ...
              const countQuery = `SELECT COUNT(*) AS total FROM \`${table.name}\`${options.where[table.name] ? ` WHERE ${options.where[table.name]}` : ''}`;
              const countResult = yield newConnection.query(countQuery);
              const totalRows = countResult[0].total;
              const chunkSize = 1000; // Number of rows to fetch at a time
              let offset = 0;
              ...
              while (offset < totalRows) {
                  const where = options.where[table.name]
                      ...
                      : '';
                  const query = `SELECT * FROM \`${table.name}\`${where} LIMIT ${chunkSize} OFFSET ${offset}`;
                  // log.info(`Running ${query}`);

                  // eslint-disable-next-line no-await-in-loop
                  yield new Promise((resolve, reject) => {
                      const rowQueue = [];
                      const chunkQuery = connection.query(query);
                      // Stream the data to the file
                      chunkQuery.on('result', (row) => {
                  ...
                  // Add a 100ms delay before the next query
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (options.dump.data !== false) {
                ...
                connection = yield DB.connect(deepmerge.all([options.connection, { multipleStatements: true }]));
                const tables = res.tables;
                res.tables = yield getDataDump(options.connection, options.dump.data, tables, options.dumpToFile, connection);

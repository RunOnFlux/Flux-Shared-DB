...
async function validateApp(appName, ip, port = 16127) {
  try {
    const result = await axios.get(`http://${ip}:${port}/apps/listallapps`, { timeout: 3000 });
...
async function getStatus(ip, port) {
  try {
    return new Promise((resolve) => {
      const client = io.connect(`http://${ip}:${port}`, { transports: ['websocket'], reconnection: false, timeout: 6000 });
